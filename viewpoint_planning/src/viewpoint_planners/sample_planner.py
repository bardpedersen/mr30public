...
class SamplePlanner:
    ...
    def next_best_view(self, N):
        ...
        for i in range(view_samples.shape[0]):
            ...
            gains.append(loss)
            gain_image_list.append(gain_image)

        # select the viewpoint with the highest gain, lowest loss
        index = np.argmin(gains)
        self.viewpoint = view_samples[index]
        #print("List of gains: ", gains)
        gain_image = gain_image_list[index]
        ...
        return self.viewpoint, gains[index], self.num_samples
