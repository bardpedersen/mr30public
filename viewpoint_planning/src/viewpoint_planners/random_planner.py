...
class RandomPlanner:
    ...
    def start_params(self, start_pose: np.array, target_params: np.array) -> None:
        ...
        # Store as numpy array for viewpoint sampling
    ...
    def next_best_view(self, N) -> np.array:
        """
        Sample a viewpoint at random
        :param N: Number of viewpoints to track in history
        :return: viewpoint, gain, total_samples
        """        
        # Convert PyTorch tensors to NumPy arrays
        camera_bounds_np = self.camera_bounds.cpu().numpy()
        target_params_np = self.target_params.cpu().numpy()
        
        # Fix: Slice the camera bounds to only include the x,y,z bounds (first 3 columns)
        # The random_sampler expects bounds of shape (2, 3): [[min_x, min_y, min_z], [max_x, max_y, max_z]]
        camera_bounds_sliced = camera_bounds_np[:, :3]  # Take only first 3 columns
        
        # Make sure target is just a 1D array [x, y, z]
        target_sliced = target_params_np
        if len(target_params_np.shape) > 1:
            target_sliced = target_params_np.flatten()[:3]
        
        viewpoint = self.view_sampler.random_sampler(
            camera_bounds_sliced, target_sliced,
        )
        ...
        # Compute gain for this viewpoint
        camera_position = self.camera_params[:3].detach()
        ...
        # Display gain image and viewpoint
        self.rviz_visualizer.visualize_gain_image(gain_image)
        self.rviz_visualizer.visualize_viewpoint(numpy_to_pose(viewpoint))
        
        rospy.loginfo(f"Selected random viewpoint with gain: {loss}")
        
        return self.viewpoint, loss , self.num_samples
