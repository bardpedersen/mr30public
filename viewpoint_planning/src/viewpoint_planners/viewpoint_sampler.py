...
class ViewpointSampler:
    ...
    def __init__(self, num_samples: int = 1, seed: int = 2024, inner_bounds_in=None):
        """
    ...
    def enforce_inner_bounds(self, position):
        """
        Ensure position is outside the inner bounds (keep-out zone)
        :param position: Camera position to check
        :return: Adjusted position that's outside inner bounds
        """
        # Return immediately if no inner bounds
        if hasattr(self, 'inner_bounds_in') and self.inner_bounds_in is not None:
            min_bounds = self.inner_bounds_in[0]
            max_bounds = self.inner_bounds_in[1]
        else:
            # We don't have enough information to calculate bounds
            return position
            
        inside = np.all((position >= min_bounds) & (position <= max_bounds))
        
        if inside:
            # If inside, calculate distances to each face of the bounding box
            distances = np.zeros(6)
            distances[0] = position[0] - min_bounds[0]  # distance to -x face
            distances[1] = max_bounds[0] - position[0]  # distance to +x face
            distances[2] = position[1] - min_bounds[1]  # distance to -y face
            distances[3] = max_bounds[1] - position[1]  # distance to +y face
            distances[4] = position[2] - min_bounds[2]  # distance to -z face
            distances[5] = max_bounds[2] - position[2]  # distance to +z face
            ...
            if min_idx == 0:
                adjusted_position[0] = min_bounds[0]
            elif min_idx == 1:
                adjusted_position[0] = max_bounds[0]
            elif min_idx == 2:
                adjusted_position[1] = min_bounds[1]
            elif min_idx == 3:
                adjusted_position[1] = max_bounds[1]
            elif min_idx == 4:
                adjusted_position[2] = min_bounds[2]
            elif min_idx == 5:
                adjusted_position[2] = max_bounds[2]
    ...
    def random_sampler(self, bounds, target) -> None:
        """
        Generate random viewpoints
        :return: None
        """
        position = np.random.uniform(bounds[0], bounds[1])
        position = self.enforce_inner_bounds(position)
        orientation = look_at_rotation(position, target)
    ...
    def semi_cylindrical_sampler(
        ...
        target: np.array = np.array([1.0, 0.0, 1.15]),
    ) -> None:
        """
    ...
    def five_dof_sampler(
        ...
        for _ in range(self.num_samples):
            ...
            camera_pos = self.enforce_inner_bounds(camera_pos)
            position = camera_pos
    ...
    def random_neighbour_sampler(
        ...
        for _ in range(self.num_samples):
            ...
            camera_pos = self.enforce_inner_bounds(camera_pos, target_pos)
            position = camera_pos
            # Calculate the rotation quaternion to make the camera look at the target
            orientation = look_at_rotation(camera_pos, target_pos)
    ...
    def predefine_start_pose(
        self,
        target_position: np.array = np.array([0.4, 0.0, 0.2]),
        distance: float = -0.20, # starting distance from the object in y-axis
    ) -> np.ndarray:
        """
        ...
        orientation = look_at_rotation(position, target_position, ref)
        pose = np.concatenate((position, orientation))
        return pose
