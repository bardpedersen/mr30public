...
class Perceiver:
    ...
    def run(self):
        ...
        segmentation_mask = self.color_segmentation(color_image, camera_info)
        # Get semantics
        semantics = self.assign_semantics(camera_info, segmentation_mask)
        self.capturer.publish_point_cloud()
        return depth_image, points, semantics

    def color_segmentation(self, color_image: np.array, camera_info) -> np.array:
        """
        ...
        segmentation_mask = cv2.resize(
            segmentation_mask, (camera_info.width, camera_info.height), interpolation=cv2.INTER_NEAREST
        )
