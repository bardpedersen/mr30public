...
class RealsenseROSCapturer:
    ...
    def __init__(self):
        ...
        self.points = None
        self.unfilter_pointcloud = None
        self.camera_info = None
        # Realsense ROS topics.
        self.use_sim = rospy.get_param("/use_sim_time", True)
        rospy.Subscriber(
            "camera/color/image_raw", Image, self.color_callback, queue_size=2
        )
        rospy.Subscriber(
        ...
        self.point_publish = rospy.Publisher("camera/depth/color/points_pose", PointCloud2, queue_size=1)
    ...
    def publish_point_cloud(self):
        # Publish point cloud
        if self.unfilter_pointcloud is not None:
            self.point_publish.publish(self.unfilter_pointcloud)
