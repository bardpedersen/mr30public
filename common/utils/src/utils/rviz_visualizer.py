...
class RvizVisualizer:
    def __init__(self):
        ...
        self.inner_bounds_pub = rospy.Publisher("inner_bounds", Marker, queue_size=1)
    ...
    def publish_pose_array(self, topic_name, positions, color=(1.0, 0.0, 0.0, 1.0)):
        """
        Visualize a trajectory as a line strip
        
        :param topic_name: Topic name to publish the trajectory
        :param positions: Array of positions (N, 3)
        :param color: RGBA color tuple for the line (r, g, b, a)
        """
        if not hasattr(self, 'line_publishers'):
            self.line_publishers = {}
            
        if topic_name not in self.line_publishers:
            self.line_publishers[topic_name] = rospy.Publisher(
                topic_name, Marker, queue_size=1
            )
        
        marker = Marker()
        marker.header.frame_id = self.world_frame_id
        marker.header.stamp = rospy.Time.now()
        marker.ns = "particle_path"
        marker.id = 0
        marker.type = Marker.LINE_STRIP
        marker.action = Marker.ADD
        marker.pose.orientation.w = 1.0
        marker.scale = Vector3(0.01, 0.01, 0.01)  # Line width
        ...
        self.line_publishers[topic_name].publish(marker)
    ...
    def visualize_frontiers(self, frontier_points: np.array) -> None:
        """
        Visualize frontier voxels in Rviz as a separate PointCloud2 message
        :param frontier_points: (N, 3) array of frontier point coordinates
        """
        assert frontier_points.shape[1] == 3, "The input array must have shape (N, 3)"
        
        # Bright magenta color for frontiers
        color_frontier = struct.unpack("I", struct.pack("BBBB", 255, 0, 255, 255))[0]
        
        # Create structured array for point cloud
        points_dtype = np.dtype([
            ("x", np.float32), ("y", np.float32), ("z", np.float32), ("rgb", np.uint32),
        ])
        points_arr = np.empty(frontier_points.shape[0], dtype=points_dtype)
        points_arr["x"] = frontier_points[:, 0]
        points_arr["y"] = frontier_points[:, 1]
        points_arr["z"] = frontier_points[:, 2]
        points_arr["rgb"] = color_frontier
        
        # Convert to PointCloud2 and publish
        frontier_cloud = ros_numpy.point_cloud2.array_to_pointcloud2(
            points_arr, rospy.Time.now(), self.world_frame_id
        )
        self.frontier_pub.publish(frontier_cloud)
    ...
    def visualize_inner_bounds(self, bounds: np.array) -> None:
        """
        Visualize the camera bounds in Rviz as a Marker message
        :param bounds: (2, 3) array of camera bounds
        """
        ...
        marker.scale = Vector3(*size)
        marker.color = ColorRGBA(0.0, 0.0, 1.0, 0.3)
        self.inner_bounds_pub.publish(marker)
